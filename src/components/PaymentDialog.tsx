...
import { addPayment } from '@/store/invoiceStore';
...
export function PaymentDialog({ open, onOpenChange, invoice, onSaved }: PaymentDialogProps) {
  ...
  const handleSubmit = () => {
    ...
    if (parsedAmount > maxAmount) {
      toast.error(`Valor não pode exceder ${formatCurrency(maxAmount)}`);
      return;
    }
    ...
    if (result) {
      toast.success(
        parsedAmount >= maxAmount
          ...
          : `Pagamento de ${formatCurrency(parsedAmount)} registrado`
      );
      setAmount('');
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="rounded-lg bg-secondary/50 p-3 space-y-1 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-mono text-status-paid">{formatCurrency(invoice.totalPaid)}</span>
          </div>
        ...
        <div className="space-y-3">
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <Button type="button" variant="link" size="sm" className="h-auto p-0 text-xs" onClick={handlePayFull}>
                Pagar total
              </Button>
            </div>
            <Input
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="isEarly" className="text-sm font-normal cursor-pointer">
        ...
        <DialogFooter>
          ...
          >
            {confirming ? 'Confirmar Pagamento' : 'Registrar'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
